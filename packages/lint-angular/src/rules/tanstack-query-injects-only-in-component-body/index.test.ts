...
import tanstackQueryInjectsOnlyInComponentBody from "./index.js";

await run({
  name: "tanstack-query-injects-only-in-component-body",
  rule: tanstackQueryInjectsOnlyInComponentBody,
  valid: [
    ...
    'import { Component } from "@angular/core"; import { injectQuery } from "@tanstack/angular-query-experimental"; @Component({}) class C { query = injectQuery(() => ({ queryKey: ["x"], queryFn })); }',
    {
      code: 'import { Component } from "@angular/core"; import { injectQuery } from "@tanstack/angular-query-experimental"; @Component({}) class C { query = injectQuery(() => ({ queryKey: ["x"], queryFn })); }',
      options: [{ tanstackQuerySources: ["@tanstack/angular-query"] }],
    },
  ...
  invalid: [
    ...
    {
      code: 'import { injectQuery } from "@tanstack/angular-query-experimental"; function load() { return injectQuery(() => ({ queryKey: ["x"], queryFn })); }',
      errors: ["onlyInComponentBody"],
    },
    {
      code: 'import { injectQuery } from "@tanstack/custom-query"; function load() { return injectQuery(() => ({ queryKey: ["x"], queryFn })); }',
