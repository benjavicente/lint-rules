...
function isNgModuleDecorator(context: Context, node: AnyNode): boolean {
  ...
  if (callee?.type === "Identifier") {
    return getImportedName(context, callee, "@angular/core") === "NgModule";
  }

  return (
    callee?.type === "MemberExpression" &&
    callee.object?.type === "Identifier" &&
    isNamespaceImport(context, callee.object, "@angular/core") &&
    getPropertyName(callee.property) === "NgModule"
  );
}

function getNgModuleMetadata(context: Context, node: AnyNode): AnyNode | null {
...
function isModuleCallFromImport(
  ...
  if (moduleRef?.type === "Identifier") {
    return getImportedName(context, moduleRef, expectedSource) === expectedModuleName;
  }
  ...
  createOnce(context: Context) {
    return {
      Decorator(node) {
        ...
        const metadata = getNgModuleMetadata(context, node as AnyNode);
        if (!metadata) return;
        ...
        if (!allowForProviding) {
          for (const call of importCalls) {
            ...
            if (
              methodName === "forRoot" &&
              isModuleCallFromImport(context, call, "@angular/router", "RouterModule") &&
              !allowForRouting
            ) {
              context.report({
                node: call,
            ...
            if (
              methodName === "forRoot" &&
              isModuleCallFromImport(context, call, "@ngrx/store", "StoreModule")
            ) {
              context.report({
                node: call,
            ...
            if (
              methodName === "forRoot" &&
              isModuleCallFromImport(context, call, "@ngrx/effects", "EffectsModule")
            ) {
              context.report({
                node: call,
            ...
            if (
              ...
              isModuleCallFromImport(context, call, "@ngrx/store-devtools", "StoreDevtoolsModule")
            ) {
              context.report({
                node: call,
        ...
        if (!allowForRouting) {
          for (const call of importCalls) {
            ...
            if (!isModuleCallFromImport(context, call, "@angular/router", "RouterModule")) {
              continue;
            }
            context.report({
              node: call,
